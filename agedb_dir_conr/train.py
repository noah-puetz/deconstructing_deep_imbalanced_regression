...
parser.add_argument('--data_dir', type=str, default='data', help='data directory') # Adjusted for new dataset location
parser.add_argument('--model', type=str, default='resnet50', help='model name') 
parser.add_argument('--store_root', type=str, default='checkpoint_conr', help='root path for storing checkpoints, logs')
parser.add_argument('--store_name', type=str, default='', help='experiment store name')
parser.add_argument('--device', type=str, default=None)
parser.add_argument('--gpu', type=int, default=0) # Adjuestment necessary for MPS implementation
...
parser.add_argument('--lr', type=float, default=1e-3, help='initial learning rate') # There is a difference 
...
# ConR
parser.add_argument('--conr', type=bool, default=False, help='whether to enable conr')
...
parser.add_argument('--name', type=str, default=f'model_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}', help='name of the experiment')
...
if args.seed is None:
    ...
    print(f"Random seed not specified, using {args.seed}")
else:
...
args.store_name = f"{args.dataset}_{args.model}{args.store_name}_{args.optimizer}_{args.loss}_{args.lr}_{args.batch_size}_seed{args.seed}"
...
if args.pretrained == "latest":
    print("Loading latest backbone checkpoint...")
    dir_key = f'./{args.store_root}/'
    all_subdirs = [d for d in os.listdir(dir_key) if os.path.isdir(os.path.join(dir_key, d))]    
    print(f"Subdirs: {all_subdirs}")
    all_subdirs = [f'{dir_key}{d}' for d in all_subdirs if "backbone" in d]    
    latest_subdir = max(all_subdirs, key=os.path.getmtime)
    args.pretrained = latest_subdir + '/ckpt.best.pth.tar'
    print(f"Latest subdir: {args.pretrained}")
...
def main():
    ...
    model_param_size = get_model_param_size(model)
    print(f"Model size: {model_param_size}")
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        train_loss = train(train_loader, model, optimizer, epoch, train_labels, criterion)
        val_loss_mse, val_loss_l1, val_loss_gmean,_ = validate(val_loader, model, train_labels=train_labels)
    ...
    # Need to be adjusted to append to the file
    ...
    with open(args.output_csv, mode='a') as outputs_file:
        ...
        to_write = [args.name,test_loss_l1,shot_dict['many']['l1'],shot_dict['median']['l1'],shot_dict['low']['l1'],test_loss_gmean,shot_dict['many']['gmean'],shot_dict['median']['gmean'],shot_dict['low']['gmean']]
        outputs.writerow(to_write)


def train(train_loader, model, optimizer, epoch, train_labels, criterion):
    ...
    if args.bmse:
        ...
        metrics.extend([noise_var, l2])
    
    if args.conr:
        loss_c = AverageMeter("Loss(ConR)", ":.3f")
        metrics.append(loss_c)
    ...
    for idx, (inputs,imgs_c, targets, weights) in enumerate(train_loader):
        ...
        inputs, targets, weights = inputs.to(args.device, non_blocking=True), targets.to(args.device, non_blocking=True), weights.to(args.device, non_blocking=True)
        ...
        if args.conr:
            if args.device == "cuda" and args.gpu is not None:
                ...
                imgs_c[0] = imgs_c[0].to(args.device, non_blocking=True)
                imgs_c[1] = imgs_c[1].to(args.device, non_blocking=True)
            ...
            outputs_c = outputs_c.clone().detach()
        ...
        if args.conr:
            l_c= ConR(features_c,targets_c,outputs_c,w=args.w,weights =weights_c,t=args.t,e=args.e)
            loss+=args.beta*l_c
    ...
    if args.fds and epoch >= args.start_update:
        ...
        encodings, labels = torch.from_numpy(np.vstack(encodings)).to(args.device), torch.from_numpy(np.hstack(labels)).to(args.device)
