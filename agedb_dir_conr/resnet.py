...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, bucket_num=100, bucket_start=3, start_update=0, start_smooth=1,
                 kernel='gaussian', ks=5, sigma=2, momentum=0.9, device=None):
        ...
        self.kernel_window = self._get_kernel_window(kernel, ks, sigma, self.device)
        self.half_ks = (ks - 1) // 2
    ...
    @staticmethod
    def _get_kernel_window(kernel, ks, sigma, device='cpu'):
