...
def age_bin_metrics(preds, labels):
    # Convert predictions and labels to numpy arrays if necessary.
    ...
    # Dictionary to hold metrics for each age bin.
    metrics = {}
    
    # For each unique age value in labels create a bin.
    for age in np.unique(labels):
        mask = labels == age  # Create a boolean mask for the current age.
        if np.sum(mask) == 0:
            continue
        
        # Compute error for the current bin.
        errors = preds[mask] - labels[mask]
        
        # Calculate metrics.
        mse = np.mean(errors ** 2)
        mae = np.mean(np.abs(errors))
        gmean_val = gmean(np.abs(errors))
        
        # Save metrics for this age bin.
        metrics[age] = {'mse': mse, 'mae': mae, 'gmean': float(gmean_val)}
    
    return metrics

def save_age_bin_metrics_to_csv(preds, labels, model_name, csv_path="evaluation_results.csv"):
    # Compute metrics
    metrics = age_bin_metrics(preds, labels)
    
    # Prepare rows for the DataFrame
    rows = []
    for age, metric in metrics.items():
        row = {
            "model": model_name,
            "age": age,
            "mse": metric["mse"],
...
def query_yes_no(question, default="yes"):
    ...
    If default is given, it's used when input isn't available or in non-interactive mode.
    """
    ...
    # Add a try-except block to handle EOF error
    try:
        ...
        if choice == '':
            ...
            return query_yes_no(question, default)
    except (EOFError, KeyboardInterrupt):
        print("\nAssuming default answer due to non-interactive environment.")
        return valid[default]
