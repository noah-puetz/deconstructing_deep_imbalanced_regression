...
parser.add_argument('--data_dir', type=str, default='data', help='data directory') # Adjusted for new data location
parser.add_argument('--model', type=str, default='resnet50', help='model name')
parser.add_argument('--store_root', type=str, default='checkpoint_vir', help='root path for storing checkpoints, logs')
parser.add_argument('--store_name', type=str, default='', help='experiment store name')
parser.add_argument('--gpu', type=int, default=0) # Changed
...
parser.add_argument('--seed', type=int, default=42, help='randomness seed')  # 1223

parser.add_argument('--age_bins', action='store_true', default=False, help='whether to use age bins')
parser.add_argument('--age_bins_csv_path', type=str, default='age_bins.csv', help='Path to the CSV file containing age bins')
...
if args.resume == "latest":
    ...
    all_subdirs = [d for d in os.listdir(args.store_root) if os.path.isdir(os.path.join(args.store_root, d))]        
    print(f"All subdirs: {all_subdirs}")
    all_subdirs = [f'{args.store_root}/{d}' for d in all_subdirs]
    latest_subdir = max(all_subdirs, key=os.path.getmtime)
    args.resume = glob.glob(f'{latest_subdir}/ckpt.best.pth.tar')[0]
    print(f"Latest checkpoint: {args.resume}")
...
args.store_name = f"{args.lambda_reg}_{args.seed}_{args.lambda_recons}_{args.dataset}_{args.model}{args.store_name}_{args.optimizer}_{args.loss}_{args.lr}_{args.batch_size}_seed{args.seed}"
    ...
    with torch.no_grad():
        ...
        if args.use_edl:
            ...
            if prefix == 'Test':
                ...
                if args.age_bins:
                    save_age_bin_metrics_to_csv(np.hstack(preds), np.hstack(labels), args.name, csv_path=args.age_bins_csv_path)
                    ...
                    with open(args.output_csv, mode='a') as outputs_file:
                        ...
                        to_write = [args.name,
                                    losses_l1,shot_dict['many']['l1'],shot_dict['median']['l1'],shot_dict['low']['l1'],
                                    loss_gmean,shot_dict['many']['gmean'],shot_dict['median']['gmean'],shot_dict['low']['gmean']]
...
def shot_metrics(preds, labels, train_labels, variance=None, edl_params=None, many_shot_thr=100, low_shot_thr=20):
    ...
    if args.use_edl:
        if sum(many_shot_cnt) > 0:
            ...
            shot_dict['many']['edl'] = shot_dict['many']['NLL'] = shot_dict['many']['Reg'] = 0
        ...
        if sum(median_shot_cnt) > 0:
            ...
            shot_dict['median']['edl'] = shot_dict['median']['NLL'] = shot_dict['median']['Reg'] = 0
        ...
        if sum(low_shot_cnt) > 0:
            ...
            shot_dict['low']['edl'] = shot_dict['low']['NLL'] = shot_dict['low']['Reg'] = 0

    if variance is not None:
        if len(many_shot_labels) > 0:
            many_nllgau, many_ause = eval_uncertainty(np.hstack(many_shot_labels), np.hstack(many_shot_preds),
                                                    np.hstack(many_shot_vars))
        else:
            many_nllgau, many_ause = 0, 0
        
        if len(median_shot_labels) > 0:
