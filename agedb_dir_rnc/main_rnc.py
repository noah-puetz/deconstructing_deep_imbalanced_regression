...
def parse_option():
    ...
    opt.model_path = './checkpoint_rnc/'
    opt.model_name = 'RnC_{}_{}_ep_{}_lr_{}_d_{}_wd_{}_mmt_{}_bsz_{}_aug_{}_temp_{}_label_{}_feature_{}_norm_{}_trial_{}_seed_{}_{}'. \
        format(opt.dataset, opt.model, opt.epochs, opt.learning_rate, opt.lr_decay_rate, opt.weight_decay, opt.momentum,
            opt.batch_size, opt.aug, opt.temp, opt.label_diff, opt.feature_sim, opt.normalize, opt.trial, opt.seed, timestamp)
    if len(opt.resume):
...
def set_loader(opt, g):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=opt.num_workers, pin_memory=True, drop_last=True, worker_init_fn=seed_worker, generator=g)
...
def main():
    ...
    if opt.seed is None:
        opt.seed = int(time.time() * 1000) % 2**32
        print(f"Random seed not specified, using {opt.seed}")
    else:
        print(f"Specified Seed: {opt.seed}")
